...
import { getInviteConsumeData } from '@/api/member'
export default {
  name: '',
  data() {
    return {
  ...
  methods: {
    toRulePage() {
      this.$router.push({ name: 'DiviRule', query: { ruleType: 'invite' } })
    },
    // 获取收益明细数据
    getData() {
      this.$loading.show()
      getInviteConsumeData()
        .then(res => {
          console.log('-res-', res)
